...
const myApiKey = "eb2f031fd5ca53f7364e22382953676f";	// Ersätt DIN-API-KEY med din egen API-nyckel
...
async function fetchLocation(id) {
    const url = "https://api.flickr.com/services/rest/?api_key="+ myApiKey +"&method=flickr.photos.geo.getLocation&photo_id="+ id +"&format=json&nojsoncallback=1;";
    const response = await fetch(url);
    if(!response.ok){
        console.log("Det fanns problem med att hämta positionen.");
        return;
    }
...
function showLocation(jsonData) {
    let locationData = jsonData.photo.location;
    let latitude = locationData.latitude;
    let longitude = locationData.longitude;

    largeImg.locationElem.innerText = "Latidud: " + latitude + " Longitud: " + longitude;
    fetchImgsByLocation(latitude, longitude);
} // Slut showLocation
// --------------------------------------------------
// Ajax-begäran av nya bilder
async function fetchImgsByLocation(lat, lon) {
    let url = "https://api.flickr.com/services/rest/?api_key=" + myApiKey + "&method=flickr.photos.search&lat=" + lat + "&lon="+ lon + "&per_page=5&format=json&nojsoncallback=1&has_geo=1"
    const response = await fetch(url);
    if(!response.ok){
        console.log("Det fanns problem med att hämta fler bilder.");
        return;
    }
    const data = await response.json();
    showMoreImgs(data);
